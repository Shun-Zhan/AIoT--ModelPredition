...
本电脑当前的 Windows 热点网关是 `10.98.128.1`，固件为 ESP32 固定分配
`10.98.128.50`，避免热点 DHCP 未及时分配地址的问题。其他电脑的热点网段可能不同：运行
`ipconfig`，找到热点对应的“本地连接*”IPv4 地址，并在本机 `wifi_credentials.h` 中把网关
改为该地址、把 ESP32 IP 改为同网段未占用的地址。ESP32 成功连接后会打印实际地址，同时注册为
`esp32-sensors.local`。
...
地址为 `10.98.128.50`。先在 **PowerShell** 测试 TCP 是否可达：

```powershell
Test-NetConnection 10.98.128.50 -Port 3333
```
...
不可达，先运行 `Test-NetConnection 10.98.128.50 -Port 3333` 检查热点连通性。
...
```powershell
cd C:\Users\你的用户名\Desktop\AIoT--ModelPredition
.\.venv\Scripts\dual-forecast.exe receive-esp32 --esp-host 10.98.128.50
```
