...
## 通过 USB 串口接收 ESP32 数据（默认，推荐）

烧录 [`firmware/esp32_s3_all_sensors/esp32_s3_all_sensors.ino`](firmware/esp32_s3_all_sensors/esp32_s3_all_sensors.ino) 后，保持用于烧录的 USB 线连接电脑。
当前固件默认关闭 Wi-Fi，不需要热点、IP 地址或 `wifi_credentials.h`。ESP32 每 2 秒会通过这个 USB 串口发送一行
`@TELEMETRY {JSON}`；普通调试文字会被接收器自动忽略。

注意：Arduino IDE 的串口监视器和电脑端接收程序不能同时占用同一个串口。启动接收程序前先关闭串口监视器。
...
在另一个终端启动 USB 串口接收桥接程序。当前这台 Mac 的烧录端口是：

```bash
dual-forecast receive-esp32-serial --serial-port /dev/cu.wchusbserial10
```
...
通过 USB 线连接 ESP32，并在设备管理器的“端口 (COM 和 LPT)”中记下它的 COM 号，例如 `COM3`。
关闭 Arduino 串口监视器。
...
脚本会在首次运行时自动创建 `.venv` 并安装依赖，自动识别唯一的 USB 串口；随后后台启动预测服务
与 ESP32 串口接收器，最后以没有地址栏和标签页的 Edge 全屏窗口打开 dashboard。若有多个 USB 串口，
用端口号明确启动：
...
按 `Esc` 可退出全屏；关闭 Edge 不会停止后台服务。需要停止后台服务时双击 `stop_dashboard.cmd`，或运行：
...
正常时每约 10 秒会出现 `Live dashboard updated from ESP32 telemetry.`。如果日志提示串口被占用，先关闭
Arduino 串口监视器后重启 `start_dashboard.cmd`。

若电脑没有安装在默认位置的 Edge，脚本会自动尝试 Chrome；两者都未找到时会以普通浏览器打开。

#### 手动启动

打开两个 PowerShell 窗口。下面命令不依赖 `source`，也不会误用 Anaconda 或全局 Python。
...
第二个窗口通过 USB 串口接收 ESP32 数据（将 `COM3` 换为实际端口）：

```powershell
cd C:\Users\你的用户名\Desktop\AIoT--ModelPredition
.\.venv\Scripts\dual-forecast.exe receive-esp32-serial --serial-port COM3
```
