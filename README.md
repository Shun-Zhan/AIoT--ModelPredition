...
| 采样模式 | 推荐间隔 | 使用条件 |
| --- | ---: | --- |
| `DEBUG` | 2 秒 | 传感器故障与安全调试 |
| `IRRIGATION_MONITORING` | 2–5 秒 | 灌溉候选、高蒸散或水阀已打开 |
| `NORMAL_MONITORING` | 30–120 秒 | 环境稳定的正常监测 |
| `NIGHT_ECO` | 5–15 分钟 | 低光、低风、土壤稳定且没有灌溉候选 |

采样策略只是“电脑端推荐 + ESP32 白名单执行”，不是实测功耗结论。固件**故意不使用 Deep Sleep**：水阀打开时必须保持 2–5 秒监测，且 ESP32 仍要接收关阀命令、维持 8 秒主机心跳超时关阀和最长开阀保护。设备独立运行或重启后使用 `OFFLINE_LOGGING` / 5 分钟；Dashboard 接收器连接后会立即下发 RAM-only 的 `IRRIGATION_MONITORING` / 5 秒配置。传感器每 5 秒更新网页，但 LittleFS 仍只每 5 分钟写入一次。
...
固件上电默认以 `OFFLINE_LOGGING` 模式每 5 分钟采集一次，并把**完整样本**写进 ESP32 的 LittleFS Flash。风速、气压、空气温湿度、土壤、两路太阳辐射任一读取失败时，这一条不入库，设备每 15 秒重新采，直到下一条完整样本出现；成功后再恢复 5 分钟周期。Flash 采用两个 4032 条的轮换文件，可断电保存约 28 天，写满时滚动覆盖最早 14 天。该离线日志不依赖电脑或网络。重新连接电脑后，运行 `dual-forecast offline-log --serial-port <端口>`，即可交互式查看状态、校验并导出 CSV，或者二次确认后擦除历史并立即启动新一轮采集。启动新一轮采集时会保存电脑当前系统时间，后续记录按设备实际经过时间累加；导出包含 `recordedAtEpochMs`、带时区的 `recordedAt` 和时间来源。设备没有 RTC，因此断电后且未重新校时的记录会明确标为估算时间。记录不会自动补传进 SQLite/预测历史。
