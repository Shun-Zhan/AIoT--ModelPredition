...
演示固件烧录后，网页中的模型状态应显示 `预测正常`、`288/288`，历史来源显示为演示数据；这不是实时传感器历史。演示模式下网页、语音和 ESP32 使用同一组演示预测与农田参数，正式“开始灌溉”可以完整走过本地安全审核、目标水量计算和流量闭环；演示模式允许重复演示，不等待正式固件的 15 分钟灌溉冷却。流量计没有脉冲时仍会在 8 秒后安全关阀，并允许下一次重新建立脉冲基线。正式灌溉是否开阀仍由 ESP32 本地安全审核决定。
...
正式灌溉按目标升数闭环：目标 V = ET₀ × Kc × A ÷ η（L），ET₀ 取下一小时预报，演示默认面积 0.1 m²、Kc=1.15、η=0.90、450 脉冲/L，目标约 32.12 mL。达到目标升数立即关阀（`volume_reached_closed`），开阀 8 秒无新脉冲判 `FLOW_FAULT` 并安全关阀，按目标升数闭环为主、命令时长（1–60 秒）作为最终硬超时兜底。调试开阀仍固定 5 秒，不参与流量闭环。
...
设备通过 HTTPS 直接调用火山引擎网关，并验证根证书；禁止使用 `setInsecure()`。云端网络/TLS/JSON/服务失败时只返回离线状态，采集、预测和本地安全闭环继续运行。正式固件的灌溉建议返回后保留 15 分钟人工确认窗口；演示固件保留 2 小时，避免评委讲解过程中自然失效，旧建议仍不能绕过设备安全审核。正式部署前应作废历史聊天、终端或截图中泄露过的 Key。
